...
from cinemanio.relations.factories import (
...
class RelationsQueryTestCase(ListQueryBaseTestCase, RelationsTestMixin):
    ...
    def get_relate_vars(self, rel):
        ...
        return (
            f"{model_name}RelationNode",
            ", ".join(rel.codes),
            f"{model_name}RelationCountNode",
            ", ".join(rel.codes),
        )
    ...
    def test_relate_first_time(self, factory, node, relation, codes, queries_count):
        ...
        self.assert_response_relation_and_counts(
            result["relate"]["relation"], result["relate"]["count"], relation(), codes
        )
    ...
    def test_change_relation(self, factory, node, relation, codes, queries_count):
        ...
        self.assert_response_relation_and_counts(result["relate"]["relation"], result["relate"]["count"], rel, codes)
    ...
    def test_object_relation(self, factory, node, codes):
        ...
        self.assert_response_relation_and_counts(
            result[query_name]["relation"], result[query_name]["relationsCount"], rel, codes
        )

    @parameterized.expand([(MovieFactory, MovieNode, MovieRelation), (PersonFactory, PersonNode, PersonRelation)])
    def test_object_no_relation(self, factory, node, relation):
        ...
        self.assert_response_relation_and_counts(
            result[query_name]["relation"], result[query_name]["relationsCount"], relation(), []
        )
    ...
    def test_objects_relation(self, factory, node, codes):
        ...
        for obj in result[query_name]["edges"]:
            self.assert_response_relation_and_counts(obj["node"]["relation"], obj["node"]["relationsCount"], rel, codes)

    @parameterized.expand([(MovieFactory, MovieRelation), (PersonFactory, PersonRelation)])
    def test_objects_no_relation(self, factory, relation):
        ...
        self.assertEqual(len(result[query_name]["edges"]), 100)
        for obj in result[query_name]["edges"]:
            self.assert_response_relation_and_counts(obj["node"]["relation"], obj["node"]["relationsCount"], rel, [])

    @parameterized.expand([(MovieRelationFactory, ["fav", "like", "seen"]), (PersonRelationFactory, ["fav", "like"])])
    def test_objects_relation_unauth(self, factory, codes):
        ...
        with self.assertNumQueries(2):
            result = self.execute(self.objects_relation_query % self.get_objects_vars(rel), None, self.get_context())

        self.assertEqual(len(result[query_name]["edges"]), 100)
        for obj in result[query_name]["edges"]:
            self.assert_unauth_response_relation_and_counts(
                obj["node"]["relation"], obj["node"]["relationsCount"], rel, codes
            )

    @parameterized.expand([(MovieRelationFactory,), (PersonRelationFactory,)])
    ...
    @parameterized.expand([(MovieRelationCountFactory,), (PersonRelationCountFactory,)])
