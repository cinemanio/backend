...
class AuthTestCase(UserQueryBaseTestCase):
    token_auth_mutation = '''
    ...
    def test_get_token_for_active_user(self):
        ...
        with self.assertNumQueries(1):
            result = execute(self.token_auth_mutation, dict(username=self.user.username, password=self.password))
    ...
    def test_get_token_for_inactive_user(self):
        ...
        with self.assertRaises(AssertionError):
            execute(self.token_auth_mutation, dict(username=self.user.username, password=self.password))
