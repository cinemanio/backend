...
class ImdbSyncTest(VCRMixin, BaseTestCase, ImdbSyncMixin):
    ...
    def test_sync_movie_matrix(self):
        imdb_movie = ImdbMovieFactory(
            id=133_093, movie__year=None, movie__title="", movie__genres=[], movie__languages=[], movie__countries=[]
        )
    ...
    def test_search_and_sync_right_person_by_movie(self, search_person):
        ...
        ImdbMovieFactory(id=1_672_719, movie=CastFactory(person=person1).movie)
        ImdbMovieFactory(id=2_702_724, movie=CastFactory(person=person2).movie)
        ImdbMovieFactory(id=1_985_034, movie=CastFactory(person=person3).movie)
        sync_person(person1.id)
        sync_person(person2.id)
        sync_person(person3.id)
        self.assertEqual(person1.imdb.id, 930_009)
        self.assertEqual(person2.imdb.id, 8_050_010)
        self.assertEqual(person3.imdb.id, 4_613_572)
    ...
    def test_search_and_sync_right_movie_by_person(self, search_movie):
        ...
        ImdbPersonFactory(id=2_357_819, person=CastFactory(movie=movie1).person)
        ImdbPersonFactory(id=3_167_230, person=CastFactory(movie=movie2).person)
        sync_movie(movie1.id)
        sync_movie(movie2.id)
        self.assertEqual(movie1.imdb.id, 1_085_492)
        self.assertEqual(movie2.imdb.id, 3_505_782)
    ...
    def test_get_movie_runtime_different_format(self):
        ...
        imdb_movie = ImdbMovieFactory(id=1_524_546)
        imdb_movie.sync()
    ...
    def test_movie_genres_adams_family(self):
        ...
        self.assertQuerysetEqual(
            imdb_movie.movie.genres.all(), ["Black and white", "Comedy", "Family", "Horror", "Series"]
        )
    ...
    def test_movie_exists_genres_adams_family(self):
        ...
        imdb_movie.sync()
        self.assertQuerysetEqual(
            imdb_movie.movie.genres.all(), ["Black and white", "Comedy", "Documentary", "Family", "Horror", "Series"]
        )
    ...
    def test_add_roles_to_movie_by_imdb_id(self):
        imdb_person1 = ImdbPersonFactory(id=905_152)  # Lilly Wachowski: director, scenarist, producer
        imdb_person2 = ImdbPersonFactory(id=206)  # Keanu Reeves: Neo
        imdb_person3 = ImdbPersonFactory(id=50390)  # Jeremy Ball: Businessman
        imdb_movie = ImdbMovieFactory(id=133_093)
    ...
    def test_add_imdb_id_to_persons_of_movie(self):
        imdb_movie = ImdbMovieFactory(id=133_093)
        cast1 = CastFactory(
            movie=imdb_movie.movie, person__first_name_en="Lilly", person__last_name_en="Wachowski", role=self.director
        )
        cast2 = CastFactory(
            movie=imdb_movie.movie, person__first_name_en="Keanu", person__last_name_en="Reeves", role=self.actor
        )
        cast3 = CastFactory(
            movie=imdb_movie.movie, person__first_name_en="Jeremy", person__last_name_en="Ball", role=self.actor
        )
        imdb_movie.sync(roles=True)
    ...
    def test_add_movies_to_writer(self):
        # writer, Dostoevskiy
        imdb_movie = ImdbMovieFactory(id=475_730)
        imdb_person = ImdbPersonFactory(id=234_502, person__country=None)
    ...
    def test_add_roles_to_person_by_imdb_id(self):
        ...
        imdb_movie2 = ImdbMovieFactory(id=108_399)  # True Romance: Clifford Worley
        imdb_person = self.imdb_dennis_hopper()
        imdb_person.sync(roles=True)
    ...
    def test_add_authors_to_movie(self):
        imdb_person = ImdbPersonFactory(id=234_502)  # writer, Dostoevskiy
        imdb_movie = ImdbMovieFactory(id=475_730)
        imdb_movie.sync(roles=True)
    ...
    def test_movie_title_en(self):
        imdb_movie = ImdbMovieFactory(id=190_332)
        imdb_movie.sync()
