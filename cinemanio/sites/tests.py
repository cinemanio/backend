...
class SitesSyncTest(VCRMixin, BaseTestCase, ImdbSyncMixin, KinopoiskSyncMixin):
    ...
    def test_sync_all_roles_of_person_from_imdb_and_kinopoisk(self):
        ...
        self.assertListEqual(
            list(Movie.objects.filter(kinopoisk=None).values_list("title_en", flat=True)),
            ["Made for Each Other", "Bad Boy's 10th Anniversary... The Hits", "Screen Test: Dennis Hopper"],
        )
        self.assertListEqual(
            list(Movie.objects.filter(imdb=None).values_list("title_en", flat=True)),
            [
        ...
        movie = Movie.objects.get(imdb__id=172_794)
    ...
    def test_sync_all_roles_of_ru_movie_from_imdb_and_kinopoisk(self):
        ...
        role = movie.cast.get(person__kinopoisk__id=185_619)
